...
NO_VERSION_FUNCTIONS = ["_request_options_api", "_request_futures_data_api"]
...
def fetch_endpoints():
    ...
    for url in URLS:
        ...
        for code in all_code_blocks:
            ...
            if (
                len(parts) >= 2
                and parts[0] in ["GET", "POST", "PUT", "DELETE"]
                ...
                if (
                    any(endpoint.startswith(prefix) for prefix in DEPRECATED_PREFIXES)
                    or (method, endpoint) in DEPRECATED_ENDPOINTS
                ):
    ...
    valid_endpoints = {
        ...
        if any(endpoint.startswith(prefix) for prefix in PREFIX_MAP)
    }
...
def get_request_function_and_path(
    endpoint: str,
...
def convert_to_function_name(method: str, endpoint: str) -> str:
    ...
    request_function, cleaned_endpoint, version = get_request_function_and_path(
        endpoint
    )
...
def check_function(
    method, request_function, cleaned_endpoint, version_arg, params=None
):
    """
    ...
    if method != "GET":
        ...
        try:
            ...
            client_function(
                method.lower(), cleaned_endpoint, signed=False, **version, **params
            )
            return False
            ...
            if "signature" in e.message or "API-key" in e.message:
                return True
            if (
                "The endpoint has been out of maintenance" in e.message
                or "This endpoint has been deprecated, please remove as soon as possible."
                in e.message
                ...
                print(
                    f"Error calling endpoint {request_function} - {cleaned_endpoint} - {version_arg} - {params}"
                )
                return None
...
def main():
    ...
    print(
        f"{len(new_endpoints)} endpoints were added out of {len(endpoints)} scrapped in client.py"
    )
    ...
    for method, endpoint in new_endpoints_async:
        generate_function_code(
            method, endpoint, type="async", file_name="./binance/async_client.py"
        )

    print(
        f"{len(new_endpoints_async)} endpoints were added out of {len(endpoints)} scrapped in async_client.py"
    )
