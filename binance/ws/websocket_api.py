...
class WebsocketAPI(ReconnectingWebsocket):
    ...
    def __init__(self, url: str, tld: str = 'com', testnet: bool = False):
        ...
        self._connection_lock = asyncio.Lock()  # used to ensure only one connection is established at a time
        super().__init__(
            url=url,
            prefix="",
            path="",
    ...
    def _handle_message(self, msg):
        ...
        if 'status' in parsed_msg:
            if parsed_msg['status'] != 200:
                throwError = True
                exception = BinanceAPIException(parsed_msg, parsed_msg['status'], json.dumps(parsed_msg['error']))
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        for req_id in response_ids:
            ...
            if not future.done():
                future.set_exception(BinanceWebsocketUnableToConnect("WebSocket closing"))
