...
import asyncio
import time
from .conftest import api_key, api_secret, proxy
import pytest
import sys
import logging
...
def test_many_symbols_small_queue(exchange_symbols):
    logger.debug("Starting test_many_symbols_small_queue with queue size 1")
    twm = ThreadedWebsocketManager(
        api_key, api_secret, https_proxy=proxy, testnet=True, max_queue_size=1
    )
    ...
    try:
        logger.debug("Starting ThreadedWebsocketManager")
        twm.start()
        logger.debug("Starting multiplex socket with %d streams", len(exchange_symbols))
        twm.start_multiplex_socket(callback=handle_message, streams=exchange_symbols)
...
def test_many_symbols_adequate_queue(exchange_symbols):
    logger.debug("Starting test_many_symbols_adequate_queue with queue size 200")
    twm = ThreadedWebsocketManager(
        api_key, api_secret, https_proxy=proxy, testnet=True, max_queue_size=200
    )
    ...
    try:
        ...
        twm.start()
        logger.debug(
            "Starting futures multiplex socket with %d streams", len(exchange_symbols)
        )
        twm.start_futures_multiplex_socket(
            callback=handle_message, streams=exchange_symbols
        )
        logger.debug("Waiting 10 seconds for messages")
        time.sleep(10)

        logger.debug(
            "Test completed. Messages received: %d, Errors: %s",
            messages_received,
            error_received,
        )
...
def test_slow_async_callback_no_error(exchange_symbols):
    logger.debug("Starting test_slow_async_callback_no_error with queue size 400")
    twm = ThreadedWebsocketManager(
        api_key, api_secret, https_proxy=proxy, testnet=True, max_queue_size=400
    )
    ...
    try:
        ...
        twm.start_futures_multiplex_socket(
            callback=slow_async_callback, streams=exchange_symbols
        )
        logger.debug("Waiting 10 seconds for messages")
        time.sleep(10)

        logger.debug(
            "Test completed. Messages processed: %d, Errors: %s",
            messages_processed,
            error_received,
        )
...
def test_no_internet_connection():
    ...
    with pytest.raises(
        RuntimeError,
        match="Binance Socket Manager failed to initialize after 5 seconds",
    ):
        twm = ThreadedWebsocketManager(
            api_key, api_secret, https_proxy=invalid_proxy, testnet=True
        )

        try:
            logger.debug(
                "Attempting to start ThreadedWebsocketManager with invalid proxy"
            )
            twm.start()
            logger.debug("Attempting to start kline socket (should fail)")
            twm.start_kline_socket(callback=lambda x: print(x), symbol="BTCUSDT")
        finally:
